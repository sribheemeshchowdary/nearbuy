...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  const safeVariableName = (value: string) => value.replace(/[^A-Za-z0-9_-]/g, "");
  const safeCssColor = (value: string | undefined) => {
    if (!value) return null;
    const trimmed = value.trim();
    if (/^#[0-9A-Fa-f]{3,8}$/.test(trimmed)) return trimmed;
    if (/^(rgb|hsl)a?\(\s*[\d.\s,%+-]+(?:\/\s*[\d.]+%?)?\s*\)$/.test(trimmed)) return trimmed;
...
${colorConfig
  .map(([key, itemConfig]) => {
    ...
    const variableName = safeVariableName(key);
    return color && variableName ? `  --color-${variableName}: ${color};` : null;
  })
  .filter(Boolean)
  .join("\n")}
}
